...
const relationshipsSlice = createSlice({
  ...
  reducers: {
    ...
    addRelationship: (
      state,
      action: PayloadAction<CreateRelationshipPayload>
    ) => {
      const id = nanoid();
    ...
    updateRelationship: (
      ...
      if (relationship) {
        ...
        if (
          updates.sourceTableId &&
          updates.sourceTableId !== relationship.sourceTableId
        ) {
          // Remove from old source table
          const oldSourceRelationships =
            state.bySourceTable[relationship.sourceTableId];
        ...
        if (
          updates.targetTableId &&
          updates.targetTableId !== relationship.targetTableId
        ) {
          // Remove from old target table
          const oldTargetRelationships =
            state.byTargetTable[relationship.targetTableId];
    ...
    deleteRelationshipsByTable: (state, action: PayloadAction<string>) => {
      ...
      // Update allIds
      state.allIds = state.allIds.filter(
        id => !uniqueRelationships.includes(id)
      );
    ...
    // Visual adjustments
    updateRelationshipControlPoints: (
      state,
      action: PayloadAction<{
        id: string;
        controlPoints: Array<{
          ...
          x: number;
          y: number;
          type: import('../../types').ControlPointType;
        }>;
      }>
    ) => {
